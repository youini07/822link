...
function readSettings(): Record<string, any> {
  try {
    ...
    console.error('[Telegram] 설정 파일을 읽는 중 오류:', error)
  }
  return {}
}
...
async function executeDirectPipeline(sessionData: {
  ...
  try {
    ...
    // 5. 로컬 원본 이미지 파일들 static/images 폴더로 최종 아카이빙(정리이동)
    // [3번 해결] 업로드가 완료되면 원본 6개 파일 전부를 static/images 폴더로 이동시켜 홈페이지 R2 동기화와 구조를 통일함
    const finalProdCode = uploadRes.prodCode || prodCodeSeed
    
    // 홈페이지 정적 파일 경로 (uploadTarget에 따라 분기)
    const staticBaseDir = currentTarget === 'dreamstudio'
      ? 'c:\\Users\\youin\\OneDrive\\바탕 화면\\dreamstudiovtg\\static'
      : 'c:\\Users\\youin\\OneDrive\\바탕 화면\\catalog_app\\static'
    const outputDir = path.join(staticBaseDir, 'images', finalProdCode)
