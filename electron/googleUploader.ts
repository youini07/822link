...
export async function uploadProductToGoogle(
  ...
  try {
    // [중요] 외부 배열(sessionData.files) 덮어쓰기 방지를 위한 얕은 복사
    if (payload.localFilePaths) {
      payload.localFilePaths = [...payload.localFilePaths]
    }
    const authClient = await authenticateWithOAuth(onProgress)
    ...
    const staticImagesArr: string[] = []

    // --- 썸네일 자동 생성 로직 시작 ---
    let thumbSourcePath = payload.synthesisFilePath; 
    if (!thumbSourcePath || !fs.existsSync(thumbSourcePath)) thumbSourcePath = payload.nukkiFilePath;
    if (!thumbSourcePath || !fs.existsSync(thumbSourcePath)) thumbSourcePath = payload.localFilePaths[0];

    if (thumbSourcePath && fs.existsSync(thumbSourcePath)) {
      try {
        const localThumbDir = require('path').join(require('electron').app.isPackaged ? require('path').dirname(require('electron').app.getPath('exe')) : require('path').join(require('electron').app.getAppPath(), '..'), '822shop', 'static', 'thumbnails');
        if (!fs.existsSync(localThumbDir)) {
          fs.mkdirSync(localThumbDir, { recursive: true });
        }
        const { nativeImage } = require('electron');
        const img = nativeImage.createFromPath(thumbSourcePath);
        ...
        const thumbDest = require('path').join(localThumbDir, `${numericCode}.jpg`);
    ...
    // 1. 누끼 이미지가 존재한다면 배열 1순위로 추가 (상세이미지 최상단 노출)
    if (payload.nukkiFilePath && fs.existsSync(payload.nukkiFilePath)) {
      const ext = require('path').extname(payload.nukkiFilePath) || '.jpg'
      staticImagesArr.push(`"/static/images/${numericCode}/main${numericCode}${ext}"`)
    }

    // 2. 복사가 완료된 원본 1~6 이미지 추가
    payload.localFilePaths.forEach((p, index) => {
      if (p) {
        const fileExt = require('path').extname(p) || '.jpg'
        staticImagesArr.push(`"/static/images/${numericCode}/${index + 1}${fileExt}"`)
      }
    })
...
export async function deleteProductEntirely(
  ...
  try {
    ...
    if (driveRes.data.files && driveRes.data.files.length > 0) {
      for (const targetFolder of driveRes.data.files) {
        if (targetFolder.id) {
          ...
          console.log(`[Google Uploader] 드라이브 폴더 [${prodCode}] 완전 삭제 완료 (ID: ${targetFolder.id})`)
        }
      }
    } else {
      ...
      for (const folderPath of localPaths) {
        if (fs.existsSync(folderPath)) {
          ...
          for (const file of files) {
            // 상품코드로 시작하는 파일 (예: 570_1.jpg, 570.jpg 등) 또는 폴더 삭제
            if (file.startsWith(prodCode.toString())) {
              ...
              try {
                ...
                console.log(`[Google Uploader] 로컬 파일/폴더 삭제 완료: ${fullPath}`);
              } catch (e) {
                console.error(`[Google Uploader] 로컬 삭제 오류 (${fullPath}):`, e);
              }
            }
          }
        }
    ...
    console.error('[deleteProductFromGoogle Error]', err)
    return { success: false, error: err.message }
  }
}

/**
 * 사입품목 이미지 폴더를 확인하고, 루트에 있는 이미지들을 MMDD 형식의 폴더로 이동한 뒤 구글 드라이브에 업로드합니다.
 */
export async function uploadPurchasedImages(
  folderPath: string,
  ...
  try {
    if (!fs.existsSync(folderPath)) {
      return { success: false, count: 0, error: '사입품목 폴더가 존재하지 않습니다.' };
    }
    ...
    if (filesToUpload.length === 0) {
      onProgress('[시스템] 업로드할 새로운 사입품목 이미지가 없습니다.');
      return { success: true, count: 0 };
    }

    const now = new Date();
    const yearStr = String(now.getFullYear()).slice(-2) + '년';
    const monthStr = String(now.getMonth() + 1) + '월';
    const mmdd = String(now.getMonth() + 1).padStart(2, '0') + String(now.getDate()).padStart(2, '0');
    
    const localDateFolder = path.join(folderPath, yearStr, monthStr, mmdd);
    if (!fs.existsSync(localDateFolder)) {
      fs.mkdirSync(localDateFolder, { recursive: true });
    }

    // Move files to local MMDD folder
    for (const file of filesToUpload) {
      const oldPath = path.join(folderPath, file);
      const newPath = path.join(localDateFolder, file);
      fs.renameSync(oldPath, newPath);
    }
    
    onProgress(`[시스템] ${filesToUpload.length}개의 파일을 ${mmdd} 폴더로 이동했습니다. 구글 드라이브 업로드를 시작합니다.`);

    const authClient = await authenticateWithOAuth(() => {});
    const drive = google.drive({ version: 'v3', auth: authClient });
    const parentFolderId = parseDriveFolderId(targetDriveUrl);

    if (!parentFolderId) {
       return { success: false, count: 0, error: '구글 드라이브 URL이 유효하지 않습니다.' };
    }

    // Check if MMDD folder exists on Drive
    const query = `name='${mmdd}' and '${parentFolderId}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false`;
    const driveRes = await drive.files.list({ q: query, fields: 'files(id, name)', spaces: 'drive' });
    
    let driveDateFolderId = '';
    if (driveRes.data.files && driveRes.data.files.length > 0) {
      driveDateFolderId = driveRes.data.files[0].id!;
      onProgress(`[시스템] 구글 드라이브에 이미 ${mmdd} 폴더가 존재합니다. 해당 폴더에 업로드합니다.`);
    } else {
      const folderRes = await drive.files.create({
        requestBody: { name: mmdd, mimeType: 'application/vnd.google-apps.folder', parents: [parentFolderId] },
        fields: 'id'
      });
      driveDateFolderId = folderRes.data.id!;
      onProgress(`[시스템] 구글 드라이브에 ${mmdd} 폴더를 생성했습니다.`);
    }

    let successCount = 0;
    for (let i = 0; i < filesToUpload.length; i++) {
      const fileName = filesToUpload[i];
      const filePath = path.join(localDateFolder, fileName);
      onProgress(`[업로드] (${i + 1}/${filesToUpload.length}) ${fileName} ...`);
      ...
      await drive.files.create({
        requestBody: { name: fileName, parents: [driveDateFolderId] },
    ...
    onProgress(`[시스템] ${successCount}개의 이미지 업로드가 완료되었습니다.`);
    return { success: true, count: successCount };
  } catch (err: any) {
    console.error('uploadPurchasedImages error:', err);
