...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  google: {
    ...
    fetchSpreadsheetList: (spreadsheetUrl: string) => ipcRenderer.invoke('fetch-spreadsheet-data', spreadsheetUrl),
    /** 시트에서 다음 상품코드 채번 */
...
// ──── 호환성 레이어: 오픈마켓(MarketUploadPanel) 등 구버전 코드 지원 ────
contextBridge.exposeInMainWorld('electron', {
  getCatalogSettings: () => ipcRenderer.invoke('get-catalog-settings'),
  saveCatalogSettings: (settings: any) => ipcRenderer.invoke('save-catalog-settings', settings),
  checkBunjangSession: () => ipcRenderer.invoke('bunjang:checkSession'),
  checkJoongnaSession: () => ipcRenderer.invoke('joongna:checkSession'),
  fetchSpreadsheetData: (url: string, tab?: string) => ipcRenderer.invoke('fetch-spreadsheet-data', url, tab),
  uploadBunjangMulti: (codes: string[], url: string) => ipcRenderer.invoke('bunjang:uploadMulti', { prodCodes: codes, spreadsheetUrl: url }),
  uploadFruitsMulti: (codes: string[], url: string) => ipcRenderer.invoke('fruits:uploadMulti', { prodCodes: codes, spreadsheetUrl: url }),
  uploadJoongnaMulti: (codes: string[], url: string) => ipcRenderer.invoke('joongna:uploadMulti', { prodCodes: codes, spreadsheetUrl: url }),
  deleteBunjangMulti: (codes: string[], url: string) => ipcRenderer.invoke('bunjang:deleteMulti', { prodCodes: codes, spreadsheetUrl: url }),
  deleteFruitsMulti: (codes: string[], url: string) => ipcRenderer.invoke('fruits:deleteMulti', { prodCodes: codes, spreadsheetUrl: url }),
  deleteJoongnaMulti: (codes: string[], url: string) => ipcRenderer.invoke('joongna:deleteMulti', { prodCodes: codes, spreadsheetUrl: url }),
  startBulkUp: (items: any) => ipcRenderer.invoke('bulk-up-products', items),
  cancelMarketUpload: () => ipcRenderer.invoke('market:cancelAll'),
  deleteSpreadsheetRows: (opts: any, rows: any, tab?: string) => ipcRenderer.invoke('google:delete-product-entirely', { prodCode: rows[0]?.productCode }), // 임시 폴백
  onMarketProgress: (callback: (data: any) => void) => {
    const handler = (_event: any, data: any) => callback(data);
    ipcRenderer.on('market:progress', handler);
    return () => ipcRenderer.removeListener('market:progress', handler);
  },
  onMarketLog: (callback: (msg: string) => void) => {
    const handler = (_event: any, msg: string) => callback(msg);
    ipcRenderer.on('market:log', handler);
    return () => ipcRenderer.removeListener('market:log', handler);
  }
})
