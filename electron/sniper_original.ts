...
export class SniperManager {
    ...
    async syncServerTime() {
        try {
            ...
            const response = await fetch('https://band.us', { method: 'GET' });
            const end = performance.now();
            const serverDateStr = response.headers.get('date');
            if (serverDateStr) {
                const serverTime = new Date(serverDateStr).getTime();
                const latency = (end - start) / 2;
                this.timeOffset = (serverTime + latency) - Date.now();
                console.log(`[TimeSync] Naver server time synced. Offset: ${this.timeOffset.toFixed(2)}ms, Ping: ${(latency * 2).toFixed(2)}ms`);
            }
        }
        catch (e) {
            ...
            for (const [id, task] of this.tasks.entries()) {
                ...
                if (bandSetting.deadlineType === 'OBSERVER') {
                    ...
                    if (timeRemaining <= 125000 && timeRemaining > 0 && task.status === 'WAITING') {
                        task.status = 'WARMUP_60';
                        this.emitUpdate(task);
                        (task.strategy === 'FEED' ? this.executeFeedSnipe(task) : this.executeDirectSnipe(task)).catch(e => {
    ...
    async waitForTriggerAndSnipe(task, bandSetting) {
        ...
        try {
            ...
            for (let attempt = 0; attempt < 4; attempt++) {
                ...
                await targetPage.waitForTimeout(100);
                await targetPage.waitForTimeout(100);
                await targetPage.keyboard.press('Control+Enter').catch(() => { });
                const textLeft = await input.evaluate((el) => {
                    return el.value || el.textContent || '';
                }).catch(() => '');
                if (!textLeft || !textLeft.includes(nextBid.toString().replace(/['"]/g, ''))) {
                ...
                if (tieResult.isTied && !tieResult.amIFirst) {
                    ...
                    if (task.maxBid === undefined || emergencyBid <= task.maxBid) {
                        ...
                        try {
                            ...
                            await emergencyInput.pressSequentially(emergencyBid.toString(), { delay: 10, timeout: 2000 }).catch(async () => {
                                await emergencyInput.fill(emergencyBid.toString(), { timeout: 2000 });
                            });
                            let sent = false;
                            for (let attempt = 0; attempt < 4; attempt++) {
                                await targetPage.waitForTimeout(100);
                                await targetPage.waitForTimeout(100);
                                await targetPage.keyboard.press('Control+Enter').catch(() => { });
                                const textLeft = await emergencyInput.evaluate((el) => {
                                    return el.value || el.textContent || '';
                                }).catch(() => '');
                                if (!textLeft || !textLeft.includes(emergencyBid.toString().replace(/['"]/g, ''))) {
                                    sent = true;
                                    break;
                                }
                            }
    ...
    // --- Time Based Workflow (Direct URL Reload Architecture) ---
    async executeDirectSnipe(task) {
        ...
        const context = await browser.newContext({ storageState: this.storagePath });
        const page = await context.newPage();
        // 1. 다이렉트 주소로 바로 접속 (피드 스크롤 탐색 제거)
        const targetPostUrl = task.url; // 예: .../post/5
        console.log(`[Task ${task.id}] 상세 게시글 다이렉트 접속: ${targetPostUrl}`);
        await page.goto(targetPostUrl, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(async (e) => {
            console.log(`[Task ${task.id}] goto failed, retrying once...`, e.message);
            await page.waitForTimeout(2000);
        ...
        try {
            // 2. 바디 전체를 컨테이너로 사용 (단일 게시글 페이지이므로 버튼 찾기 생략)
            const containerSelector = 'body';
            const postContainer = page.locator(containerSelector);
            // WARMUP 60s (캐시 최적화: 1분 전 2회 강제 새로고침)
            const warmup60Time = task.targetTime - 60000;
            await waitUntill(warmup60Time);
            task.status = 'WARMUP_60';
            this.emitUpdate(task);
            console.log(`[Task ${task.id}] 마감 1분 전: 캐시/네트워크 최적화 1차 워밍업 (2회 새로고침)`);
            for (let i = 0; i < 2; i++) {
                await page.reload({ waitUntil: 'domcontentloaded' }).catch(() => { });
                await page.waitForTimeout(500);
            }
            // WARMUP 30s (캐시 최적화: 30초 전 3회 강제 새로고침)
            const warmup30Time = task.targetTime - 25000;
            await waitUntill(warmup30Time);
            task.status = 'WARMUP_30';
            this.emitUpdate(task);
            console.log(`[Task ${task.id}] 마감 30초 전: 캐시/네트워크 최적화 2차 워밍업 (3회 새로고침)`);
            for (let i = 0; i < 3; i++) {
                await page.reload({ waitUntil: 'domcontentloaded' }).catch(() => { });
                await page.waitForTimeout(500);
            }
            // ★ FINAL SNIPE: 마감 2.5초 전 다이렉트 새로고침 돌격 로직
            // 실제 테스트 결과 새로고침~엔터까지 약 0.5초가 소요되므로,
            // 2.5초 전에 출발하면 마감 2.0초 전에 완벽하게 꽂힘 (1.5초는 네트워크 지연 대비 버퍼)
            const snipeTime = task.targetTime - 2700;
            await waitUntill(snipeTime);
            task.status = 'SNIPING';
            this.emitUpdate(task);
            console.log(`[Sniper] 마감 2.5초 전: 강력 새로고침(Reload) 돌격 시작!`);
            await this.waitForCommentsAPI(page, containerSelector, async () => {
                await page.reload({ waitUntil: 'domcontentloaded' }).catch(e => console.error("Reload error:", e));
            });
            await page.waitForTimeout(100);
            // 숨겨진 댓글 모두 로드 ("이전 댓글 더보기" 자동 클릭)
            ...
            // ★ 핵심 로직: 다이렉트 주소 새로고침 직후의 화면에서 100% 최신 댓글만 파싱
            let currentHighest = 0;
            for (let j = commentsText.length - 1; j >= 0; j--) {
                const parsed = this.parseBidAmount(commentsText[j]);
                if (parsed !== null) {
            ...
            console.log(`[Sniper FINAL] 최종 최고가 산정: ${currentHighest}`);
            let nextBid = currentHighest === 0 ? task.startBid : currentHighest + task.increment;
            if (task.maxBid !== undefined && nextBid > task.maxBid) {
                task.status = 'ABORTED';
            ...
            try {
                ...
                const targetSendTime = task.targetTime - 1000;
                const timeRemaining = targetSendTime - this.getServerTime();
                if (timeRemaining > 0) {
                    console.log(`[Sniper DIRECT] 마감 1.0초 전 전송 대기 중... (${timeRemaining}ms)`);
                ...
                const submitTime = this.getServerTime();
                task.submitTimeLog = submitTime;
                // ★ 테스트 결과 반영: 800ms -> 300ms로 단축하여 방어 로직 즉시 가동
                await page.waitForTimeout(100);
            }
            catch (e) {
                console.error("Submit failed:", e);
                throw new Error(`입찰 전송 실패: ${e.name === 'TimeoutError' ? '댓글창이 닫혔거나 요소를 찾을 수 없습니다.' : e.message}`);
            }
            task.status = 'SUCCESS';
            const diff = task.submitTimeLog ? (task.submitTimeLog - task.targetTime) : 0;
            task.resultMsg = `[낙찰 성공] 방어 성공! (차이:${diff}ms) 완료 -> 확인사살 돌입🏁`;
            this.emitUpdate(task);
            // ★ 동점 감지 (1회 제한 안전장치)
            // 안전장치 3중: (1) myNickname 필수 (2) maxBid 필수 (3) 최대 1회
            const bandSettingForTie = task.bandSettingId ? this.bandSettings.get(task.bandSettingId) : undefined;
            const myNameForTie = bandSettingForTie?.myNickname || '';
            if (myNameForTie) {
                // 입찰 직후 즉시 리프레쉬하여 동점 확인 (다이렉트 주소이므로 reload)
                await page.reload({ waitUntil: 'domcontentloaded' }).catch(() => { });
                await page.waitForFunction((sel) => {
                    const container = document.querySelector(sel);
                    if (!container)
                        return false;
                    return container.querySelectorAll('.cComment, .commentItem, [class*="comment_item"]').length > 0;
                }, containerSelector, { timeout: 3000 }).catch(() => { });
                await page.waitForTimeout(100); // 텍스트 렌더링 대기
                const { authors: tieAuthors, texts: tieTexts } = await this.extractAuthorsAndTexts(postContainer);
                const tieResult = this.checkTieBreak(tieAuthors, tieTexts, myNameForTie);
                if (tieResult.isTied && !tieResult.amIFirst) {
                    // 동점인데 내가 늦었다 → 즉시 +1 재입찰! (최대 1회)
                    const emergencyBid = tieResult.highest + task.increment;
                    if (task.maxBid === undefined || emergencyBid <= task.maxBid) {
                        console.log(`[TieBreak-Emergency] 동점 추월! ${tieResult.highest} → ${emergencyBid} 재입찰`);
                        const emergencyInput = postContainer.locator(inputSelector).first();
                        try {
                            ...
                            await page.waitForTimeout(100);
                            nextBid = emergencyBid;
                            task.resultMsg = `[동점 추월] ${tieResult.highest} 동점 → ${emergencyBid} 재입찰! (차이:${diff}ms)`;
                            this.emitUpdate(task);
                        }
                        catch (e) {
                            console.error('[TieBreak-Emergency] Submit failed:', e);
            ...
            else if (!myNameForTie) {
                console.log('[TieBreak] myNickname 미설정 → 동점 체크 건너뜀');
            }
            else if (task.maxBid === undefined) {
                console.log('[TieBreak] maxBid 미설정(무제한) → 안전장치: 동점 체크 비활성화');
            }
            // 20초 연장 방어 (피드 구조를 넘겨줌)
    ...
    async executeFeedSnipe(task) {
        ...
        try {
            ...
            task.status = 'WARMUP_30';
            this.emitUpdate(task);
            // ★ FINAL SNIPE: 마감 2.0초 전 본게임 타격 로직 (최적화)
            const snipeTime = task.targetTime - 2500;
            await waitUntill(snipeTime);
            console.log(`[Sniper FEED] 마감 2.5초 전: 본게임 타격 접기/펴기 콤보 시전!`);
            await commentBtn.click().catch(() => { }); // 접기 (또는 펴기)
            await page.waitForTimeout(100); // 리액트 DOM 파괴 애니메이션 대기
            await this.waitForCommentsAPI(page, containerSelector, async () => {
                await commentBtn.click().catch(() => { }); // 다시 펴기 (최신 네트워크 요청 발송)
            });
            await page.waitForTimeout(100);
            ...
            try {
                ...
                const timeRemaining = targetSendTime - this.getServerTime();
                if (timeRemaining > 0) {
                    console.log(`[Sniper FEED] 마감 1.0초 전 전송 대기 중... (${timeRemaining}ms)`);
                    await page.waitForTimeout(timeRemaining);
                }
                let sent = false;
                for (let attempt = 0; attempt < 4; attempt++) {
                ...
                if (tieResult.isTied && !tieResult.amIFirst) {
                    ...
                    if (task.maxBid === undefined || emergencyBid <= task.maxBid) {
                        try {
                            await input.focus({ timeout: 2000 });
                            // React 상태 업데이트 지연을 방지하기 위해 순차 입력 시도
                            await input.pressSequentially(emergencyBid.toString(), { delay: 10, timeout: 2000 }).catch(async () => {
                                await input.fill(emergencyBid.toString(), { timeout: 2000 });
                            });
                            let sent = false;
                            for (let attempt = 0; attempt < 4; attempt++) {
                                if (attempt > 0)
