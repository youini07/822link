...
export default function MeasuredSizeInputs({ groupId, isAnalyzing }: MeasuredSizeInputsProps) {
  ...
  return (
    <div className="mt-5 flex flex-wrap items-center gap-6 px-5 py-4 bg-[#fdfdfd] border border-gray-200/80 rounded-xl shadow-sm animate-fade-in transition-all">
      {/* 1. 가슴(허리)단면 입력 필드 */}
      <div className="flex items-center gap-2">
        <span className="text-xs font-bold text-gray-600">가슴(허리)단면:</span>
        <input 
          type="text" 
          placeholder="예: 65"
          value={width} 
          ...
          className="w-16 border-b-2 border-gray-300 focus:border-[#0078d4] bg-transparent text-center text-sm font-semibold text-gray-800 outline-none transition-colors disabled:opacity-50"
        />
      </div>

      {/* 2. 총장 입력 필드 */}
      <div className="flex items-center gap-2">
        <span className="text-xs font-bold text-gray-600">총장:</span>
        <input 
          type="text" 
          placeholder="예: 44"
          value={length} 
      ...
      <div className="flex items-center gap-2 w-full">
        <span className="text-xs font-bold text-gray-600 shrink-0">제품 결함 (하자):</span>
        <input 
          type="text" 
          ...
          disabled={isAnalyzing}
          className="flex-1 border-b-2 border-gray-300 focus:border-[#0078d4] bg-transparent text-sm font-medium text-gray-800 outline-none transition-colors disabled:opacity-50 px-1 py-0.5"
        />
      </div>

      {/* 5. 상품 상태 선택 셀렉트 */}
      <div className="flex items-center gap-2 w-full mt-2">
        <span className="text-xs font-bold text-gray-600 shrink-0">상품 상태:</span>
        <select 
          value={condition} 
