...
export default function DashboardPage() {
  ...
  // 1. 앱 마운트 시 기존 감시 폴더 설정(localWatchPath)을 불러와서 워처(Watcher) 명시적 재시작 요청
  // - main.ts의 ready-to-show 시점에 발송되는 file:initial 이벤트를 React 리스너 등록 전이라 놓치는 문제를 방지
  useEffect(() => {
    if (!window.electronAPI) return
    window.electronAPI.settings.get('localWatchPath').then((path) => {
      if (path) {
        window.electronAPI.settings.set('localWatchPath', path)
      }
    })
  }, [])
  ...
  return (
    <div className="h-full flex flex-col bg-[#e8e8e8] text-[#202020] text-sm select-none p-2 gap-2 overflow-hidden">
      
      {/* ────────────────────────────────────────────── */}
      {/* 1. 제품 이미지 대시보드 전용 스마트 컨트롤 헤더 */}
      {/* ────────────────────────────────────────────── */}
      <div className="flex flex-wrap items-center justify-between gap-4 px-4 py-3 bg-white border border-gray-300 rounded-xl shadow-sm shrink-0">
        <div className="flex items-center gap-4">
          <div className="bg-[#0078d4]/10 p-2.5 rounded-lg text-[#0078d4]">
            <Layers size={22} className="animate-pulse" />
          </div>
          <div>
            <h1 className="text-base font-bold text-gray-800 tracking-tight">AI 1+5 이미지 그룹화 & 분석 터미널</h1>
            <p className="text-[11px] text-gray-500 mt-0.5">첫 대표 사진을 클릭해 지정하면 뒤따르는 5장의 제품 사진이 자동 묶음 형성됩니다.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-5 py-2.5 rounded-lg text-xs font-semibold text-gray-600 bg-white border border-gray-300 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            title="그룹 공장초기화 원복"
          >
            <RotateCcw size={14} />
            <span>재설정</span>
          ...
          <button
            ...
            disabled={isConfirmed || isAnalyzing}
            className="flex items-center gap-1.5 px-5 py-2.5 rounded-lg text-xs font-semibold text-purple-600 bg-purple-50 border border-purple-200 hover:bg-purple-100 transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-sm"
            ...
            <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-lg">
              ...
              {!isPaused ? (
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded text-xs font-bold bg-amber-500 hover:bg-amber-600 text-white transition-colors shadow-sm"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded text-xs font-bold bg-[#107c10] hover:bg-[#0b590b] text-white transition-colors shadow-sm"
                >
                  <Play size={14} className="fill-white" />
                  <span>다시 진행</span>
                </button>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded text-xs font-bold bg-red-600 hover:bg-red-700 text-white transition-colors shadow-sm"
              >
                <Square size={14} className="fill-white" />
                <span>작업종료</span>
              </button>

              <div className="px-3 py-1 flex items-center gap-2 text-xs font-semibold text-gray-700">
      ...
      {isRepresentativeMode && (
        <div className="bg-gradient-to-r from-blue-50 to-[#e9f2fc] border border-blue-200 rounded-xl px-8 py-3 flex items-center gap-2.5 text-xs text-[#0078d4] animate-fade-in shrink-0 shadow-sm">
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-2 min-h-0">
        ...
        <div className="flex-[5.5] flex flex-col gap-4 min-w-0 bg-white border border-gray-300 rounded-xl shadow-sm p-6 overflow-hidden">
          ...
          {files.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-gray-200 bg-white rounded-xl p-8 text-center text-gray-400">
              <ImageOff size={44} className="text-gray-300 mb-3" />
              <p className="font-bold text-sm text-gray-600">감시 폴더 내에 감지된 이미지가 없습니다.</p>
              <p className="text-xs text-gray-400 mt-1">설정 페이지에서 제품 이미지 폴더를 연동해 주세요.</p>
            </div>
          ) : isTelegramActive ? (
            <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-[#0078d4]/30 bg-[#f8fbff] rounded-xl p-8 text-center shadow-inner">
              ...
              <h2 className="text-lg font-bold text-gray-800 mb-2">스마트폰 원격 연동 중</h2>
              <p className="text-sm text-gray-600 font-medium">
                텔레그램을 통한 AI 전산 파이프라인이 다이렉트로 가동되고 있습니다.
              </p>
              <div className="mt-4 bg-white px-4 py-2 rounded-lg border border-blue-200 shadow-sm flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                <span className="text-xs font-bold text-gray-600">우측 하단의 시스템 로그를 확인해 주세요.</span>
              </div>
            </div>
          ) : productGroups.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-[#0078d4]/30 bg-white rounded-xl p-8 text-center">
              <Crown size={48} className="text-[#0078d4]/20 mb-3 animate-pulse" />
              <p className="font-bold text-sm text-gray-700">그룹핑이 시작되지 않았습니다.</p>
                  ...
                  <div
                    ...
                    className={`relative bg-white border rounded-xl p-6 transition-all duration-300 ${
                      isTargetAnalyzing 
                        ...
                        : isGroupCompleted
                        ? 'border-2 border-[#107c10] shadow-sm bg-green-50/5'
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-3 border-b border-gray-100 pb-2">
                      <div className="flex items-center gap-2">
                        <span className="flex items-center justify-center w-6 h-6 rounded-full bg-gray-800 text-white text-xs font-bold shadow-sm">
                          {group.index}
                        </span>
                        <h3 className="font-bold text-gray-700 text-sm">
                          제품 코드군 (순차 코드: <span className="text-[#0078d4] font-semibold">{group.members[0]?.prodCode || '대기 중'}</span>)
                        </h3>
                        <span className="text-[11px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded font-semibold">
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 w-full">
                      ...
                      <div 
                        className="w-full xl:w-44 flex flex-col items-center bg-gray-50 border border-gray-200/80 rounded-xl p-3 relative group/head shrink-0 shadow-inner transition-colors hover:bg-blue-50/30"
                        onDragOver={handleDragOverRep}
                        onDrop={(e) => handleDropRep(e, group.repFile?.path || '')}
                      >
                        <div className="w-28 h-28 xl:w-36 xl:h-36 flex items-center justify-center bg-white border border-gray-200 rounded-lg overflow-hidden relative shadow-sm">
                        ...
                        <div className="mt-2 text-center w-full min-w-0">
                          <span className="text-[11px] font-bold text-gray-800 truncate block px-1" title={group.repFile?.name}>
                            {group.repFile?.name}
                          </span>
                          <span className="text-[9px] text-gray-400 block mt-0.5 font-medium">{formatBytes(group.repFile?.size || 0)}</span>
                          <span className="text-[12px] font-extrabold text-gray-800 block mt-2 tracking-widest">메인</span>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div>
                          <div className="text-[11px] font-bold text-gray-400 tracking-wider mb-2 uppercase flex items-center gap-1">
                          ...
                          <div className={viewMode === 'details' ? 'flex flex-col gap-1.5' : 'grid grid-cols-4 sm:grid-cols-6 gap-3'}>
                              ...
                              if (viewMode === 'details') {
                                return (
                                  <div key={file.id} className="flex items-center gap-3 w-full">
                                    <div className="w-16 text-right text-[12px] font-bold text-gray-700 shrink-0">{labelText}</div>
                                    <div
                                      draggable={!isConfirmed}
                                      onDragStart={(e) => handleDragStart(e, file.path)}
                                      onDragOver={handleDragOverRep}
                                      onDrop={(e) => handleDropSubImage(e, file.path, group.groupId)}
                                      className={`flex-1 flex items-center gap-3 p-1.5 bg-white border border-gray-100 rounded-lg group shadow-sm transition-all ${
                                        isConfirmed ? 'cursor-default' : 'cursor-grab active:cursor-grabbing hover:bg-blue-50/50 hover:border-[#a3c9f7]'
                                      }`}
                                    >
                                    <div className="w-9 h-9 shrink-0 flex items-center justify-center bg-gray-50 border border-gray-200 rounded overflow-hidden relative">
                                      {!isImgErr ? (
                                    ...
                                    <div className="flex-1 flex items-center min-w-0 pr-2">
                                      <div className="w-1/3 truncate text-[11px] font-semibold text-gray-700 pr-2" title={file.name}>{file.name}</div>
                                      <div className="w-1/3 truncate text-[10px] text-gray-400 pr-2">{new Date(file.timestamp).toLocaleString()}</div>
                                      <div className="w-1/6 truncate text-[10px] text-gray-400 pr-2">{file.type || 'JPG 파일'}</div>
                                      <div className="w-1/6 truncate text-[10px] text-gray-400 text-right">{formatBytes(file.size)}</div>
                                    </div>
                                    {!isConfirmed && (
                              ...
                              return (
                                <div key={file.id} className="flex flex-col items-center gap-1 w-full">
                                  <div
                                    ...
                                    onDragOver={handleDragOverRep}
                                    onDrop={(e) => handleDropSubImage(e, file.path, group.groupId)}
                                    className={`w-full flex flex-col items-center p-2 bg-white border border-gray-100 rounded-xl relative group shadow-sm transition-all ${
                                      isConfirmed 
                                        ? 'cursor-default' 
                                        : 'cursor-grab active:cursor-grabbing hover:bg-blue-50/50 hover:border-[#a3c9f7]'
                                    }`}
                                  >
                                  <div className="w-14 h-14 sm:w-16 sm:h-16 flex items-center justify-center bg-gray-50 border border-gray-200 rounded-lg overflow-hidden relative">
                                    {!isImgErr ? (
                                      <img
                                  ...
                                  <span className="text-[10px] font-semibold text-gray-700 truncate w-full text-center mt-1.5 px-0.5" title={file.name}>
                                    {file.name}
                                  </span>
                                  </div>
                                  <span className="text-[11px] font-extrabold text-gray-800 mt-1">{labelText}</span>
                                </div>
                              )
                            })}
                            ...
                            {group.members.length <= 1 && (
                              <div className="col-span-3 sm:col-span-5 h-20 flex flex-col items-center justify-center border border-dashed border-gray-200 rounded-xl text-gray-400 bg-slate-50/30 text-xs">
                                <span>서브 이미지가 아직 배정되지 않았습니다.</span>
                                <span className="text-[10px] text-gray-400/80 mt-0.5">우측 미분류 이미지를 이곳으로 드래그해 넣으세요!</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        {group.members[0]?.aiTitle && (
                          <div className="mt-5 bg-[#e9f2fc]/50 border border-[#0078d4]/10 rounded-xl p-5 animate-fade-in">
                            ...
                            <div className="text-xs space-y-1.5 text-gray-700">
                              <div><span className="font-semibold text-gray-400 w-16 inline-block">카테고리:</span> {group.members[0].aiCategory}</div>
                              <div className="flex"><span className="font-semibold text-gray-400 w-16 shrink-0">생성 제목:</span> <span className="flex-1 break-words">{group.members[0].aiTitle}</span></div>
                              <div className="flex"><span className="font-semibold text-gray-400 w-16 shrink-0">제품 설명:</span> <div className="flex-1 break-words whitespace-pre-wrap max-h-24 overflow-y-auto pr-2 custom-scrollbar">{group.members[0].aiDescription}</div></div>
                            </div>
                          </div>
                        )}
                      </div>
        ...
        <div className="w-full md:w-96 flex flex-col gap-2 shrink-0 min-h-0">
          ...
          <div
            ...
            className={`flex-1 flex flex-col bg-white border rounded-xl overflow-hidden shadow-sm transition-all duration-300 min-h-[250px] ${
          ...
          >
            ...
            {unclassifiedImages.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center p-6 text-center text-gray-400">
                ...
                <p className="font-bold text-xs text-gray-600">모든 이미지가 그룹에 배정됨</p>
                <p className="text-[10px] text-gray-400 mt-0.5">남은 미분류 이미지가 없습니다.</p>
              </div>
            ) : (
                  ...
                  {unclassifiedImages.map((file) => {
                    ...
                    if (viewMode === 'details') {
                      return (
                        <div
                          ...
                          className={`flex items-center gap-3 p-1.5 rounded-lg border bg-white relative transition-all group ${
                            isRepModeActive 
                              ? 'cursor-pointer border-blue-200 hover:border-blue-500 hover:bg-blue-50/70 shadow-sm' 
                              : isConfirmed 
                              ? 'cursor-default border-gray-100'
                              : 'cursor-grab active:cursor-grabbing border-gray-100 hover:bg-blue-50/40 hover:border-blue-200 hover:shadow-sm'
                          }`}
                        >
                          <div className="w-9 h-9 shrink-0 flex items-center justify-center bg-slate-50 border border-gray-200/80 rounded overflow-hidden relative">
                            {!isImgErr ? (
                              <img src={getImgSrc(file.path)} alt={file.name} onError={() => handleImgError(file.path)} className="w-full h-full object-cover" />
                            ) : <Image size={14} className="text-gray-400" />}
                            {isRepModeActive && (
                    ...
                    return (
                      <div
                        ...
                        onDragStart={(e) => handleDragStart(e, file.path)}
                        onClick={() => isRepModeActive && toggleRepresentative(file.path)}
                        className={`flex flex-col items-center p-2 rounded-xl border bg-white relative transition-all group ${
                          isRepModeActive 
                            ? 'cursor-pointer border-blue-200 hover:border-blue-500 hover:bg-blue-50/70 hover:scale-105 shadow-sm' 
                            : isConfirmed 
                            ? 'cursor-default border-gray-100'
                            : 'cursor-grab active:cursor-grabbing border-gray-100 hover:bg-blue-50/40 hover:border-blue-200 hover:shadow'
                        }`}
                      >
                        <div className="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center bg-slate-50 border border-gray-200/80 rounded-lg overflow-hidden relative">
                          {!isImgErr ? (
                            <img
                              src={getImgSrc(file.path)}
                              alt={file.name}
                        ...
                        <span className="text-[9px] text-gray-700 truncate w-full text-center mt-1 px-0.5 font-medium" title={file.name}>
                          {file.name}
                        </span>
                        ...
                        {!isConfirmed && (
                          <button
                            onClick={(e) => handleTrashFile(e, file.path)}
                            className="absolute -top-1 -right-1 opacity-0 group-hover:opacity-100 p-1 bg-white border border-red-200 text-red-500 hover:bg-red-500 hover:text-white rounded-full shadow-sm transition-all"
                            title="휴지통으로 이동"
                          >
                            <Trash2 size={11} />
                          </button>
      ...
      {/* ────────────────────────────────────────────── */}
      <div className="px-6 py-2 bg-[#f3f3f3] border-t border-gray-300 text-xs text-gray-600 flex justify-between font-medium shrink-0">
        <div className="flex items-center gap-4">
