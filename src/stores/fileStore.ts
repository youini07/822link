  ...
  startAIAnalysis: async () => {
    ...
    try {
      ...
      if (window.electronAPI?.settings) {
        if (uploadTarget === 'dreamstudio') {
          gdUrl = await window.electronAPI.settings.get('googleDriveUrl_dreamstudio') || await window.electronAPI.settings.get('googleDriveUrl') || ''
          gsUrl = await window.electronAPI.settings.get('googleSpreadsheetUrl_dreamstudio') || await window.electronAPI.settings.get('googleSpreadsheetUrl') || ''
        } else {
          gdUrl = await window.electronAPI.settings.get('googleDriveUrl_822shop') || await window.electronAPI.settings.get('googleDriveUrl') || ''
          gsUrl = await window.electronAPI.settings.get('googleSpreadsheetUrl_822shop') || await window.electronAPI.settings.get('googleSpreadsheetUrl') || ''
        }
      }
    } catch (e) {
    ...
    const runNextGroup = async () => {
      ...
      try {
        ...
        let prompt = ''
        if (window.electronAPI?.settings) {
          gKey = await window.electronAPI.settings.get('geminiApiKey') || ''
